...
class _RatedPageState extends State<RatedPage> {
  ...
  //-------------getRated-------------------------------
  loadRatedResponse() async {
    var responseData =
    await http.get("http://waitbuzz.net/wallpaper/api/wallpaper/rated_wallpaper",headers: {
      "lang":"en"
    });
    var obj = json.decode(responseData.body);
    var realdata = obj["data"];
    var Rated = realdata["wallpapers"];
    return Rated;
  }
  //------------postFavoruite--------------------
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0.0,
        title:Text("التقيم",style: TextStyle(color: Colors.white),),
        centerTitle: true,
        leading: InkWell(
          onTap: (){
                    ...
                    return InkWell(
                      onTap: (){
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) =>FirstItemClick("${dataRated[index]["id"]}"),
                          ),
                        );
                      },
                      child: gridItem(dataRated[index]["name"],
                          dataRated[index]["image"],dataRated[index]["id"]),
                    );
                  }),
            )
